...
const useForumContract = () => {
    ...
    const getUpVotes = async (answerId: BigNumber) : Promise<BigNumber> => {
        const upVotes = await contract.methods.getUpVotes(answerId)
        return upVotes
    }

    const postQuestion =async (message: string, account: string) : Promise<void> => {
    ...
    const postAnswer =async (questionId:BigNumber, message: string, account: string) : Promise<void> => {
        const answer = await contract.methods.postAnswer(questionId).send({
            from: account
        })
    }
